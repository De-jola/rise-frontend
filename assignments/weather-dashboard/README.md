...
4) Make the file executable, run ```bash
chmod +x *.sh```
...
How to Use
----------
1) Run ./sysinfo.sh to get both system and network information
2) To get just system information, add the system flag. Run : ```bash
./sysinfo.sh --system```
3) To get just network information, add the network flag. Run: ```bash
./sysinfo.sh --network```
